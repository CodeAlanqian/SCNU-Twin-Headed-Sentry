  ...
  serial_driver_{new drivers::serial_driver::SerialDriver(*owned_ctx_)}
{
  ...
  sentry_pub_ = this->create_publisher<auto_aim_interfaces::msg::FromSentry>("/from_sentry", 10);
  // Detect parameter client
  detector_param_client_first =
    std::make_shared<rclcpp::AsyncParametersClient>(this, "armor_detector_first");

  detector_param_client_second =
  ...
  sentry_cmd_sub_ = this->create_subscription<geometry_msgs::msg::Twist>(
    ...
    std::bind(&RMSerialDriver::sendDataSentry, this, std::placeholders::_1));
}
  ...
  while (rclcpp::ok()) {
    try {
      ...
      if (header[0] == 0x5A) {
        ...
        ReceivePacket packet = fromVectorRv(data);
...
void RMSerialDriver::sendDataSentry(geometry_msgs::msg::Twist::SharedPtr msg)
{
  try {
    ...
    RCLCPP_ERROR(get_logger(), "Error while sending Sentry data: %s", ex.what());
    reopenPort();
  }
}
...
{
  if (!detector_param_client_first->service_is_ready()) {
    RCLCPP_WARN(get_logger(), "Service not ready, skipping parameter set");
    return;
  }

  if (!detector_param_client_second->service_is_ready()) {
    RCLCPP_WARN(get_logger(), "Service not ready, skipping parameter set");
    return;
  }
  ...
  if (
    ...
    set_param_future_ = detector_param_client_first->set_parameters(
    ...
    set_param_future_ = detector_param_client_second->set_parameters(
      {param}, [this, param](const ResultFuturePtr & results) {
        for (const auto & result : results.get()) {
          if (!result.successful) {
            RCLCPP_ERROR(get_logger(), "Failed to set parameter: %s", result.reason.c_str());
